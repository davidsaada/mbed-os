...
static const uint32_t ram_rb_protect_flag                   = 0x00000002UL;
...
int StorageLite::copy_all_records(uint8_t from_area, uint32_t to_offset,
                                  uint32_t& to_next_offset)
{
    int ret;
    int ind;
...
int StorageLite::garbage_collection()
{
    ...
    ret = copy_all_records(_active_area, to_offset, to_next_offset);
    if (ret) {
        return ret;
    }
    to_offset = to_next_offset;
                              ...
                              uint32_t flags, uint32_t& next_offset)
{
    ...
    if ((prev_flags & rollback_protect_flag) && (flags & delete_flag)) {
...
int StorageLite::build_ram_table()
{
    ram_table_entry_t *ram_table = (ram_table_entry_t *) _ram_table;
    record_header_t *header;
    ...
    while (offset < _free_space_offset) {
        ...
        // offset 0 means that the file doesn't exist
        if (!save_offset) {
...
int StorageLite::free_rollback_protect_keys()
{
...
int StorageLite::init(BlockDevice *bd, size_t max_files)
{
    ...
    MBED_ASSERT(os_ret == DEVICEKEY_SUCCESS);
